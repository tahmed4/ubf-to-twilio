...
const {ValidationError} = require('../import-lib/Exceptions.js');
...
async function uploadNewBot(client, bot) {
  ...
  async function createBot(botname, serviceUid, tasks, client){
    ...
    if (botname === null || diagram === null){  
        throw ValidationError("Missing bot name or diagram")
    }

    botname = bot.timestamp + "-" + botname

    try {
        if((await validate.serviceNameAvaialble(client, botname)) === true &&  (await validate.assistantNameAvailable(client, botname) === true)){
            var tasksAndMapping = await taskGen.generateTasks(diagram);
            
            if(tasksAndMapping === null){
                throw ValidationError("Tasks and Mapping are missing.")
            }
            ...
            throw ValidationError(`${botname} already in use.`)
        }
    } catch (e) {
